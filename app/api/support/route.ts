...
/**
 * How long to spend on a pre-diagnosis before filing without one.
 *
 * The customer is held on the confirmation screen for this, so it's a ceiling
 * rather than a target: the local model normally answers in a few seconds.
 */
 ...
 *
 * Best-effort and time-boxed: the case is what matters. A missing
 * pre-diagnosis costs an agent a few minutes, a delayed one costs the customer
 * their submission.
 *
 * This ran against Proline AI for a while, which searched the live Brain and
 * gave better answers — but it took twenty-odd seconds, which is too long to
 * hold someone on a confirmation screen, and moving it off that wait meant the
 * analysis arrived as a separate email instead of in the case. Not worth the
 * complexity for now. lib/ai/prolineBrain.ts and the dashboard's
 * /api/v1/support/diagnose are still there if it's worth revisiting.
 */
async function aiSectionFor(raw: string, budgetMs: number): Promise<string> {
  ...
  try {
    const diagnoses = await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error("ai_timeout")), budgetMs),
      ),
    ]);
...
export async function POST(request: Request) {
  ...
  if (runContext) {
    const aiSection = await aiSectionFor(runContext, AI_TIMEOUT_MS);
